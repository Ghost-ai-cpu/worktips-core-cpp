...
This is the Worktips fork of the Monero-core-cpp repository

Credit for a majority of this work goes to the MyMonero team, we have only made small modifications to accommodate some of the changes Worktips has made 
...
This repository holds the C++ source code for the Worktips/CryptoNote cryptography and protocols, plus lightwallet functions which power the official apps.

A primary goal for this repository is its future convergence and reimplementation with an essential Worktips core library.


### Contents 

* `contrib/worktips-core-custom` is a Git submodule which contains a curated subset of the official Worktips C/C++ source code, eventually to be replaced by new official Worktips core repo
...
* `worktips-core-custom` (see "Setup")
...
* Not all Monero `.cpp` files which are in `worktips-core-custom` must be included for `mymonero-core-cpp` - such as when their symbols are not required by any code called by this project. See `CMakeLists.txt` for a list of files required for compilation.
...
* `CreateTransactionErrorCode: UInt32String` defined in `worktips_transfer_utils.hpp`; to remain stable within major versions
